...
def find_mock_wiki(owner: str, repo: str, mode: str):
    return next((item for item in MOCK_DB if item["owner"] == owner and item["repo"] == repo and item["mode"] == mode), None)
...
async def generate_agent_documentation_stream(
    ...
    async def event_generator():
        ...
        if should_error:
            ...
            err_resp = BaseResponse(message="Documentation generation failed", code=500, data=error_data)
            yield f"data: {err_resp.model_dump_json()}\n\n"
        else:
            # 模拟完成
            progress_data_done = {"stage": "done", "message": "Generation finished", "wiki_info": mock_wiki_info}
            ...
            if not find_mock_wiki(owner, repo, mode):
                MOCK_DB.append({
                    ...
                    "files": final_data["files"], "total_files": final_data["total_files"]
                })
...
async def get_wiki_files(
    owner: str,
    repo: str,
    mode: AgentMode = Query(AgentMode.SUB)
):
    ...
    return BaseResponse(
        ...
        data=WikiFilesResponse(
            ...
            total_files=wiki.get("total_files", 0)
        )
    )
