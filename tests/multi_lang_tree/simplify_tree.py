...
def main(input_path):
    ...
    if isinstance(data, list):
        ...
        if "tree" in data or "name" in data:
            ...
            print(
                "Error: Dictionary root structure is missing expected keys ('tree' or 'name')."
            )
            sys.exit(1)
    else:
        print(
            f"Error: JSON root is neither a dictionary nor a list. Type found: {type(data)}"
        )
        sys.exit(1)
    ...
    for index, tree_data in enumerate(trees_to_process):
        ...
        # 生成头部和树状文本
        header_lines = generate_header(
            file_path, index if len(trees_to_process) > 1 else None
        )
