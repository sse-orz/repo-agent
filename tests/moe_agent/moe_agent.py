...
class MoeAgent:
    ...
    def _file_selection(self, max_files: int):
        ...
        exclude_dirs = {
            ...
            "target", "venv", "env", ".venv", ".env", "vendor",
            "third_party", ".pytest_cache", ".mypy_cache", ".tox",
            "htmlcov", "coverage", ".idea", ".vscode", "eggs",
            ".eggs", "lib64", "parts", ".cache"
        }
    ...
    def _score_file_importance(self, rel_path: str, size: int) -> float:
        ...
        if any(d in path_lower for d in ["src/", "lib/", "core/", "api/", "pkg/", "internal/"]):
            score += 50
        
        # Entry points (highest priority)
        if filename in ["main.py", "index.js", "app.py", "server.py", "__init__.py",
                       "main.go", "main.rs", "index.ts", "main.c", "main.cpp"]:
            score += 100
        
        # Important files
        if any(keyword in filename for keyword in ["main", "index", "app", "server", "config", "interface"]):
            score += 30
    ...
    def _module_clustering(self):
        ...
        self.module_structure = self.clusterer.cluster(
            self.selected_files
        )
        ...
        for module in self.module_structure['modules'][:10]:
            print(f"   • {module['name']}: {module['file_count']} files (Priority: {module['priority']})")
        
        if self.module_structure['total_modules'] > 10:
            print(f"   ... and {self.module_structure['total_modules'] - 10} more modules")
    ...
    def _module_documentation(self, max_workers: int=5):
        ...
        print(f"{'='*60}")
        print(f"Generating documentation for {self.module_structure['total_modules']} modules...")
        
        self.module_results = self.module_doc_agent.generate_all_modules(
            modules=self.module_structure['modules'],
        ...
        summary = {
            ...
            "successful": sum(1 for r in self.module_results if r["status"] == "success"),
        ...
        return {
            ...
            "stages": {
                ...
                "3_module_clustering": {
                    ...
                    "total_modules": self.module_structure.get("total_modules", 0) if self.module_structure else 0
                },
                "4_module_docs": {
                    "status": "success",
                ...
                "5_macro_docs": {
                    ...
                    "successful": sum(1 for r in self.macro_results if r["status"] == "success"),
                    "failed": sum(1 for r in self.macro_results if r["status"] == "error")
                },
                "6_index": {
                    "status": self.summary_result.get("status", "unknown") if self.summary_result else "error",
